...
```text
selected = ["consistency-checker", "continuity-checker", "ooc-checker", "project-style-checker"]
...
- 允许扩展字段（如 `hard_violations`、`soft_suggestions`、`positive_evidence`、`missing_positive_evidence`、`backwrite_candidates`），但不得替代必填字段
- `project-style-checker` 必须额外输出：`hard_violations`、`soft_suggestions`、`positive_evidence`、`missing_positive_evidence`、`override_eligible`、`backwrite_candidates`、`constraint_pack_hash`、`gate`
...
- `report_file`（str）
- `notes`（str）：在当前执行契约中必须是单个字符串；`selected_checkers`、`timeline_gate`、`project_style_gate`、`constraint_pack_hash`、`anti_ai_force_check`、`project_style_force_check` 等扩展信息统一压成单行文本写入此字段，不得作为独立顶层键传入
...
- **项目风格闸门（新增）**：`project-style-checker` 必须给出 `project_style_gate`。
  - `pass`：可进入 Step 4。
  - `blocked`：存在项目风格硬违规，必须先在 Step 4 修复。
  - `override_required`：仅当全是允许例外的软规则时可进入 Step 4，并要求生成 Override/债务记录。
  - `recheck_required`：本章在 Step 4 修完后必须用同一版 `constraint_pack_hash` 复检，未复检不得进入 Step 5。
- `constraint_pack_hash` 必须从 Step 1 合同透传到 Step 3 聚合结果；缺失时默认视为 `project_style_gate=blocked`。
...
**Hard Block（必须修复才能继续）**：
- `project-style-checker.hard_violations` 非空
- `gate.status = blocked`
- `constraint_pack_hash` 缺失或与 Step 1 不一致
- 缺少 `missing_positive_evidence` 且对应规则被标记为 hard

**可带债进入 Step 4，但不得直接进 Step 5**：
- `gate.status = override_required`
- 仅存在允许例外的软规则问题
- 必须在 Step 4 输出 deviation / Override / 偿还计划

**复检要求**：
- Step 4 修复后必须重新运行同一版 `project-style-checker`
- 复检产物必须带回 `project_style_force_check`
- 若复检时 `constraint_pack_hash` 已变化，直接判定为 `recheck_required`，要求重跑 Step 1-4
