...
def main() -> None:
    ...
    p_study = sub.add_parser("study", help="转发到 study_manager")
    p_study.add_argument("args", nargs=argparse.REMAINDER)
    ...
    if tool == "study":
        raise SystemExit(_run_data_module("study_manager", [*forward_args, *rest]))
    if tool == "migrate":
